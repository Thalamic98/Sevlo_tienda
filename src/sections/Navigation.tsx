...
import { ShoppingBag, X, Search, Instagram, Facebook, Twitter, Trash2, MessageCircle } from 'lucide-react';
...
const Navigation = ({ cartItems, onRemoveFromCart, onUpdateQuantity, onClearCart, onCheckout }: NavigationProps) => {
          ...
          <div className="flex flex-col h-full">
            <div className="flex items-center justify-between p-6 border-b">
              <div className="flex items-center gap-3">
                <ShoppingBag size={24} className="text-[#8b6d4b]" />
                <h3 className="font-serif text-2xl">Tu Carrito</h3>
                {totalItems > 0 && (
                  <span className="px-2 py-1 bg-[#8b6d4b] text-white text-sm rounded-full">
                    {totalItems}
                  </span>
                )}
              </div>
                ...
                <div className="space-y-6">
                  {cartItems.map((item) => (
                    <div key={item.id} className="flex gap-4 pb-6 border-b border-gray-100">
                      <div className="w-24 h-24 bg-[#fafafa] overflow-hidden rounded-lg">
                        <img
                          src={item.image}
                          alt={item.name}
                      ...
                      <div className="flex-1">
                        ...
                        <p className="text-[#8b6d4b] font-medium mt-1">${item.price.toFixed(2)} c/u</p>
                        <div className="flex items-center gap-3 mt-3">
                          <button
                            onClick={() => onUpdateQuantity(item.id, Math.max(0, item.quantity - 1))}
                            className="w-8 h-8 flex items-center justify-center border border-gray-200 hover:border-[#8b6d4b] transition-colors rounded"
                          >
                            -
                          </button>
                        ...
                        <p className="text-sm text-[#696969] mt-2">
                          Subtotal: <span className="font-medium text-black">${(item.price * item.quantity).toFixed(2)}</span>
                        </p>
                      </div>
                      <button
            ...
            {cartItems.length > 0 && (
              <div className="p-6 border-t bg-[#fafafa]">
                ...
                <div className="flex items-center justify-between mb-4">
                  <span className="text-lg text-[#696969]">Subtotal</span>
                ...
                <button 
                  ...
                  className="w-full py-4 bg-green-600 text-white font-medium tracking-wide btn-hover flex items-center justify-center gap-2 rounded-lg hover:bg-green-700 transition-colors"
                >
                ...
                <button
                  ...
                  className="w-full py-3 mt-3 text-red-500 font-medium tracking-wide hover:text-red-700 transition-colors flex items-center justify-center gap-2"
                >
                  <Trash2 size={18} />
                  Vaciar carrito
                </button>
